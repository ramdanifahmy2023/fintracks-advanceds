...
Error loading filter data.
            <Button variant="link" size="sm" className="p-0 h-auto ml-1 text-destructive underline" onClick={() => window.location.reload()}>
Refresh page
            </Button>
          </AlertDescription>
        </Alert>
    ...
    <Card className="mx-4 my-2 border-0 shadow-sm bg-card/95 backdrop-blur-sm">
      <CardContent className="p-4 px-[16px] py-0 my-0 mx-0 bg-transparent">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {(filters.platforms.length > 0 || filters.stores.length > 0 || filters.dateRange.preset !== 'thisMonth') && <Button variant="ghost" size="sm" onClick={clearAllFilters} className="h-7 px-2 text-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {datePresets.map(preset => <Button key={preset.value} variant={filters.dateRange.preset === preset.value ? "default" : "outline"} size="sm" onClick={() => handlePresetChange(preset.value)} className="h-7 px-2 text-xs">
                {preset.label}
              </Button>)}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant={filters.dateRange.preset === 'custom' ? "default" : "outline"} size="sm" className="h-7 px-2 text-xs">
...
Custom
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <div className="flex">
                    <div className="p-3 border-r">
                      ...
                      <Calendar mode="single" selected={filters.dateRange.from} onSelect={date => handleDateSelect(date, 'from')} disabled={date => date > filters.dateRange.to} initialFocus />
                    </div>
                    <div className="p-3">
                      <div className="text-xs font-medium mb-2">Sampai Tanggal</div>
                      <Calendar mode="single" selected={filters.dateRange.to} onSelect={date => handleDateSelect(date, 'to')} disabled={date => date < filters.dateRange.from} />
                    </div>
                  </div>
                </PopoverContent>
              </Popover>
          ...
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between h-8 text-xs" disabled={platformsLoading}>
                  {platformsLoading ? "Loading..." : filters.platforms.length === 0 ? "Select Platform" : `${filters.platforms.length} selected`}
                  <ChevronDown className="h-3 w-3 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-72" align="start">
                <div className="space-y-3">
                  <Input placeholder="Search platforms..." value={platformSearch} onChange={e => setPlatformSearch(e.target.value)} className="h-8 text-xs" />

                  <div className="flex gap-1">
                    <Button variant="outline" size="sm" onClick={() => onFiltersChange({
                      ...filters,
                      platforms: platforms.map(p => p.id)
                    })} disabled={platforms.length === 0} className="h-7 px-2 text-xs">
All
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => onFiltersChange({
                      ...filters,
...
None
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-1">
                    {platforms.length === 0 ? <div className="text-xs text-muted-foreground p-2">No platforms available</div> : filteredPlatforms.map(platform => <div key={platform.id} className="flex items-center space-x-2 p-1">
                      <Checkbox id={platform.id} checked={filters.platforms.includes(platform.id)} onCheckedChange={() => handlePlatformToggle(platform.id)} />
                      <Label htmlFor={platform.id} className="text-xs font-normal cursor-pointer flex-1">
            ...
            {filters.platforms.length > 0 && <div className="flex flex-wrap gap-1">
              {filters.platforms.slice(0, 2).map(platformId => {
                ...
                return platform ? <Badge key={platformId} variant="secondary" className="text-xs h-5 px-1">
                  {platform.platform_name}
                  <X className="ml-1 h-2 w-2 cursor-pointer" onClick={() => handlePlatformToggle(platformId)} />
                </Badge> : null;
              })}
              {filters.platforms.length > 2 && <Badge variant="secondary" className="text-xs h-5 px-1">
+{filters.platforms.length - 2}
          ...
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between h-8 text-xs" disabled={filters.platforms.length === 0 || storesLoading}>
                  {storesLoading ? "Loading..." : filters.platforms.length === 0 ? "Select platform first" : filters.stores.length === 0 ? "Select Store" : `${filters.stores.length} selected`}
                  <ChevronDown className="h-3 w-3 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-72" align="start">
                <div className="space-y-3">
                  <Input placeholder="Search stores..." value={storeSearch} onChange={e => setStoreSearch(e.target.value)} className="h-8 text-xs" />

                  <div className="flex gap-1">
                    <Button variant="outline" size="sm" onClick={() => onFiltersChange({
                      ...filters,
                      stores: stores.map(s => s.id)
                    })} disabled={stores.length === 0} className="h-7 px-2 text-xs">
All
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => onFiltersChange({
                      ...filters,
...
None
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-1">
                    {stores.length === 0 ? <div className="text-xs text-muted-foreground p-2">
                      {filters.platforms.length === 0 ? 'Select platform first' : 'No stores available'}
                    </div> : filteredStores.map(store => <div key={store.id} className="flex items-center space-x-2 p-1">
                      <Checkbox id={store.id} checked={filters.stores.includes(store.id)} onCheckedChange={() => handleStoreToggle(store.id)} />
                      <Label htmlFor={store.id} className="text-xs font-normal cursor-pointer flex-1">
                        {store.store_name}
            ...
            {filters.stores.length > 0 && <div className="flex flex-wrap gap-1">
              {filters.stores.slice(0, 2).map(storeId => {
                ...
                return store ? <Badge key={storeId} variant="secondary" className="text-xs h-5 px-1">
                  {store.store_name}
                  <X className="ml-1 h-2 w-2 cursor-pointer" onClick={() => handleStoreToggle(storeId)} />
                </Badge> : null;
              })}
              {filters.stores.length > 2 && <Badge variant="secondary" className="text-xs h-5 px-1">
